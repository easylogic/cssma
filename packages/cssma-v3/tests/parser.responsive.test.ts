...
describe('CSSParser - 반응형(Responsive) 기능 - Tailwind CSS 방식', () => {
  ...
  describe('기본 반응형 브레이크포인트', () => {
    it('기본 브레이크포인트를 파싱할 수 있어야 함', () => {
      ...
      expect(result?.modifierChain).toBeUndefined();
  ...
  describe('최대 너비 브레이크포인트', () => {
    it('최대 너비 브레이크포인트를 파싱할 수 있어야 함', () => {
      ...
      expect(result?.modifiers?.responsive).toEqual({ 'max-md': '@media (max-width: 768px)' });
    });
  ...
  describe('임의 브레이크포인트', () => {
    it('임의 최소 너비 브레이크포인트를 파싱할 수 있어야 함', () => {
      ...
      expect(result?.modifiers?.responsive).toEqual({ 'min-[640px]': '@media (min-width: 640px)' });
    });
    ...
    it('임의 최대 너비 브레이크포인트를 파싱할 수 있어야 함', () => {
      ...
      expect(result?.modifiers?.responsive).toEqual({ 'max-[1024px]': '@media (max-width: 1024px)' });
    });
  ...
  describe('컨테이너 쿼리 - Tailwind CSS 방식', () => {
    it('기본 컨테이너 쿼리를 파싱할 수 있어야 함', () => {
      ...
      expect(result?.modifiers?.container).toEqual({ '@md': '@media (min-width: 768px)' });
    });
    ...
    it('최대 너비 컨테이너 쿼리를 파싱할 수 있어야 함', () => {
      ...
      expect(result?.value).toBe('flex');
      expect(result?.modifierChain).toBe('@max-md');
      expect(result?.modifiers?.container).toEqual({ '@max-md': '@media (max-width: 768px)' });
    });
    ...
    it('임의 컨테이너 쿼리를 파싱할 수 있어야 함', () => {
      ...
      expect(result?.modifiers?.container).toEqual({ '@min-[320px]': '@media (min-width: 320px)' });
    });
    ...
    it('명명된 컨테이너 쿼리를 파싱할 수 있어야 함', () => {
      ...
      expect(result?.modifiers?.container).toEqual({ '@md/sidebar': '@media (min-width: 768px) and (container-type: inline-size) and (container-name: sidebar)' });
    });
