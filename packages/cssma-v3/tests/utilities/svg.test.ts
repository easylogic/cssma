...
describe('parseUtility (svg)', () => {
  describe('fill', () => {
    it('should parse Tailwind v4 fill classes', () => {
      ...
      expect(parseUtility('fill-[color:rebeccapurple]')).toEqual(baseUtility({ prefix: 'fill', value: 'color:rebeccapurple', arbitrary: true, arbitraryValue: 'color:rebeccapurple', raw: 'fill-[color:rebeccapurple]' }));
  ...
  describe('stroke', () => {
    it('should parse Tailwind v4 stroke classes', () => {
      ...
      expect(parseUtility('stroke-[color:rebeccapurple]')).toEqual(baseUtility({ prefix: 'stroke', value: 'color:rebeccapurple', arbitrary: true, arbitraryValue: 'color:rebeccapurple', raw: 'stroke-[color:rebeccapurple]' }));
  ...
  describe('stroke-width', () => {
    it('should parse Tailwind v4 stroke-width classes', () => {
      expect(parseUtility('stroke-0')).toEqual(baseUtility({ prefix: 'stroke', value: '0', numeric: true, raw: 'stroke-0' }));
      expect(parseUtility('stroke-2')).toEqual(baseUtility({ prefix: 'stroke', value: '2', numeric: true, raw: 'stroke-2' }));
      expect(parseUtility('stroke-[3px]')).toEqual(baseUtility({ prefix: 'stroke', value: '3px', arbitrary: true, arbitraryValue: '3px', raw: 'stroke-[3px]' }));
      expect(parseUtility('stroke-')).toEqual({ type: 'unknown', raw: 'stroke-' });
    });
  });
}); 