...
describe('parseUtility (outline)', () => {
  describe('outline-style', () => {
    it('should parse Tailwind v4 outline style classes', () => {
      ...
      expect(parseUtility('outline-foo')).toEqual({ type: 'unknown', raw: 'outline-foo' });
    });
  });

  describe('outline-width', () => {
    it('should parse Tailwind v4 outline width classes', () => {
      expect(parseUtility('outline-0')).toEqual(baseUtility({ prefix: 'outline', value: '0', raw: 'outline-0' }));
      expect(parseUtility('outline-2')).toEqual(baseUtility({ prefix: 'outline', value: '2', raw: 'outline-2' }));
      expect(parseUtility('outline-4')).toEqual(baseUtility({ prefix: 'outline', value: '4', raw: 'outline-4' }));
      expect(parseUtility('outline-8')).toEqual(baseUtility({ prefix: 'outline', value: '8', raw: 'outline-8' }));
      expect(parseUtility('outline-[3px]')).toEqual(baseUtility({ prefix: 'outline', value: '[3px]', raw: 'outline-[3px]' }));
      expect(parseUtility('outline-[0.5rem]')).toEqual(baseUtility({ prefix: 'outline', value: '[0.5rem]', raw: 'outline-[0.5rem]' }));
      expect(parseUtility('outline-')).toEqual({ type: 'unknown', raw: 'outline-' });
      expect(parseUtility('outline-width-foo')).toEqual({ type: 'unknown', raw: 'outline-width-foo' });
    });
  });
  ...
  describe('outline-color', () => {
    it('should parse Tailwind v4 outline color classes', () => {
      ...
      expect(parseUtility('outline-[color:rebeccapurple]')).toEqual(baseUtility({ prefix: 'outline', value: '[color:rebeccapurple]', raw: 'outline-[color:rebeccapurple]' }));
      expect(parseUtility('outline-')).toEqual({ type: 'unknown', raw: 'outline-' });
      expect(parseUtility('outline-color-foo')).toEqual({ type: 'unknown', raw: 'outline-color-foo' });
    });
  });
  ...
  describe('outline-offset', () => {
    it('should parse Tailwind v4 outline offset classes', () => {
      ...
      expect(parseUtility('outline-offset-[3px]')).toEqual(baseUtility({ prefix: 'outline-offset', value: '[3px]', raw: 'outline-offset-[3px]' }));
      expect(parseUtility('outline-offset-[0.5rem]')).toEqual(baseUtility({ prefix: 'outline-offset', value: '[0.5rem]', raw: 'outline-offset-[0.5rem]' }));
      expect(parseUtility('outline-offset-')).toEqual({ type: 'unknown', raw: 'outline-offset-' });
      expect(parseUtility('outline-offset-foo')).toEqual({ type: 'unknown', raw: 'outline-offset-foo' });
    });
  });
}); 