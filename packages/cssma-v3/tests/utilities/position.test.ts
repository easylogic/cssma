...
describe('parseUtility (position)', () => {
  describe('position', () => {
    it('should parse Tailwind v4 position classes', () => {
      ...
      expect(parseUtility('position-foo')).toEqual({ type: 'unknown', raw: 'position-foo' });
    });
  });
  ...
  describe('inset', () => {
    it('should parse Tailwind v4 inset classes', () => {
      ...
      expect(parseUtility('inset-y-8')).toEqual(baseUtility({ prefix: 'inset-y', value: '8', raw: 'inset-y-8' }));
      expect(parseUtility('inset-[10px]')).toEqual(baseUtility({ prefix: 'inset', value: '[10px]', raw: 'inset-[10px]' }));
      expect(parseUtility('-inset-2')).toEqual(baseUtility({ prefix: 'inset', value: '-2', raw: '-inset-2' }));
      expect(parseUtility('inset-')).toEqual({ type: 'unknown', raw: 'inset-' });
      expect(parseUtility('inset-foo')).toEqual({ type: 'unknown', raw: 'inset-foo' });
    });
  });

  describe('top/right/bottom/left', () => {
    it('should parse Tailwind v4 directional position classes', () => {
      expect(parseUtility('top-0')).toEqual(baseUtility({ prefix: 'top', value: '0', raw: 'top-0' }));
      expect(parseUtility('right-4')).toEqual(baseUtility({ prefix: 'right', value: '4', raw: 'right-4' }));
      expect(parseUtility('bottom-8')).toEqual(baseUtility({ prefix: 'bottom', value: '8', raw: 'bottom-8' }));
      expect(parseUtility('left-[10px]')).toEqual(baseUtility({ prefix: 'left', value: '[10px]', raw: 'left-[10px]' }));
      expect(parseUtility('-top-2')).toEqual(baseUtility({ prefix: 'top', value: '-2', raw: '-top-2' }));
      expect(parseUtility('top-')).toEqual({ type: 'unknown', raw: 'top-' });
      expect(parseUtility('right-foo')).toEqual({ type: 'unknown', raw: 'right-foo' });
    });
  });
}); 