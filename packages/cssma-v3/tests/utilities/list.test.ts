...
describe('parseUtility (list)', () => {
  describe('list-style-type', () => {
    it('should parse Tailwind v4 list style type classes', () => {
      ...
      expect(parseUtility('list-image-[url(https://example.com/list.svg)]')).toEqual(baseUtility({ prefix: 'list-image', value: '[url(https://example.com/list.svg)]', raw: 'list-image-[url(https://example.com/list.svg)]' }));
  ...
  describe('list-style-position', () => {
    it('should parse Tailwind v4 list style position classes', () => {
      ...
      expect(parseUtility('list-style-position-foo')).toEqual({ type: 'unknown', raw: 'list-style-position-foo' });
    });
  });
  ...
  describe('marker', () => {
    it('should parse Tailwind v4 marker classes', () => {
      ...
      expect(parseUtility('marker-[url(https://example.com/marker.svg)]')).toEqual(baseUtility({ prefix: 'marker', value: '[url(https://example.com/marker.svg)]', raw: 'marker-[url(https://example.com/marker.svg)]' }));
      expect(parseUtility('marker-')).toEqual({ type: 'unknown', raw: 'marker-' });
      expect(parseUtility('marker-foo')).toEqual({ type: 'unknown', raw: 'marker-foo' });
    });
  });
}); 