...
/**
 * 🎯 새로운 순서 보장 ParsedModifiers 구조
 * 
 * 배열 기반으로 modifier 순서를 보장합니다.
 * priority 값에 따라 정렬되며, 같은 priority 내에서는 배열 순서를 유지합니다.
 */
export interface ParsedModifiers {
  ...
  getModifiersByType(type: ModifierType): ParsedModifier[];
  sortByPriority(): ParsedModifier[];
  generateCSSSelector(baseSelector: string): string;
}

/**
 * ParsedModifiers 구현 클래스
 */
export class ParsedModifiersImpl implements ParsedModifiers {
  modifiers: ParsedModifier[] = [];
  ...
  getModifiersByType(type: ModifierType): ParsedModifier[] {
    return this.modifiers.filter(mod => mod.type === type);
  }

  sortByPriority(): ParsedModifier[] {
    return [...this.modifiers].sort((a, b) => a.priority - b.priority);
  }

  generateCSSSelector(baseSelector: string): string {
    // 이 메서드는 SelectorGeneratorService에서 구현될 예정
    return baseSelector;
  }

  addModifier(modifier: ParsedModifier): void {
