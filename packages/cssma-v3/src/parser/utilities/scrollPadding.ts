...
export function parseScrollPadding(
  token: string,
  ...
  const result = parseScrollPaddingCore(
    token,
    context,
    originalToken,
    negative
...
function parseScrollPaddingCore(
  token: string,
  context: CssmaContext | undefined,
  originalToken: string,
  negative: boolean
): any | null {
  // 1. context-based preset parsing
  const preset = parseContextScrollPaddingUtility({
    token,
    type: "scroll-padding",
    context,
  });
