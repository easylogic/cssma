 ...
 * ```
 */
export function applyStyles<T extends SceneNode>(node: T, styles: string = ''): T {
  ...
  const hasResize = 'resize' in node;
  ...
  // 채우기 속성
  if (hasFills && styleObject.fills !== undefined) {
    const fillableNode = node as SceneNode & { fills: ReadonlyArray<Paint> | Paint[] };
    fillableNode.fills = styleObject.fills as Paint[];
  }
  ...
  // 효과 속성
  if (hasEffects && styleObject.effects !== undefined) {
  ...
  // 불투명도 속성
  if (hasOpacity && styleObject.opacity !== undefined) {
  ...
  // 벡터 노드 특수 처리
  if (node.type === 'VECTOR' && styleObject.paths !== undefined) {
    const vectorNode = node as VectorNode;
    vectorNode.vectorPaths = styleObject.paths?.map(path => {
        return {
            data: path,
            windingRule: 'NONZERO',
        }
