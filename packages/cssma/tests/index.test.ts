...
describe('Style Processor', () => {
  describe('Layout Properties', () => {
    it('should process width and height', () => {
      const result = processCssStyles('w-[100px] h-[200px]');
    ...
    it('should process padding', () => {
      const result = processCssStyles('p-[16px] pt-[8px] pr-[24px] pb-[32px] pl-[12px]');
      expect(result).toMatchObject({
    ...
    it('should process gap', () => {
      const result = processCssStyles('gap-[16px] gap-x-[24px] gap-y-[32px]');
      expect(result).toMatchObject({
    ...
    it('should handle responsive layout with parent context', () => {
      const result = processCssStyles('w-full h-full', { parentLayoutMode: 'HORIZONTAL' });
      expect(result).toMatchObject({
  ...
  describe('Color Properties', () => {
    it('should process background color', () => {
      const result = processCssStyles('bg-[#FF0000]');
      expect(result).toMatchObject({
        fills: [{
          type: 'SOLID',
          color: { r: 1, g: 0, b: 0 }
    ...
    it('should process text color', () => {
      const result = processCssStyles('text-[#00FF00]');
      expect(result).toMatchObject({
        fills: [{
          type: 'SOLID',
          color: { r: 0, g: 1, b: 0 }
    ...
    it('should process border color', () => {
      const result = processCssStyles('border-[#0000FF]');
      expect(result).toMatchObject({
        strokes: [{
          type: 'SOLID',
          color: { r: 0, g: 0, b: 1 }
  ...
  describe('Effect Properties', () => {
    it('should process opacity', () => {
      const result = processCssStyles('opacity-[0.5]');
      expect(result).toMatchObject({
        opacity: 0.5
      });
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle invalid values', () => {
      const result = processCssStyles('w-invalid h-[] p-[abc]');
      expect(result).toEqual({
        fills: []
      });
    });
    ...
    it('should handle mixed valid and invalid values', () => {
      const result = processCssStyles('w-[100px] h-invalid bg-[#FF0000] p-[abc]');
      expect(result).toMatchObject({
        width: 100,
        layoutSizingHorizontal: 'FIXED',
        fills: [{
